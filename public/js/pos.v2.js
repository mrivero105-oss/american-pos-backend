...
export class POS {
    constructor() {
        console.log("POS v2 loaded successfully - Cache Defeated");
        this.cart = [];
        this.products = [];
        this.customers = [];
    ...
    renderCart() {
        this.dom.cartItems.innerHTML = this.cart.map(item => {
            ...
            return `
                <div class="cart-item bg-white p-3 rounded-lg border border-gray-100 flex justify-between items-center group" data-id="${item.id}">
                    <div class="h-10 w-10 rounded-lg overflow-hidden bg-slate-100 border border-slate-200 mr-3 flex-shrink-0">
                    ...
                    <div class="flex-1 min-w-0 mr-3">
                        ...
                        <div class="text-sm text-gray-500">
                            $${parseFloat(item.price).toFixed(2)} <span class="text-xs text-slate-400">(${priceBs} Bs)</span> x ${item.quantity}
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        <div class="flex items-center gap-1 bg-gray-50 rounded-lg p-1">
    ...
    holdSale() {
        ...
        this.cart = [];
        this.renderCart();
        this.checkHeldSale(); // Update button state
    ...
    restoreSale() {
        ...
        this.cart = JSON.parse(heldCart);
        localStorage.removeItem('held_cart'); // Clear held cart after restoring
        this.renderCart();
        this.checkHeldSale(); // Update button state
        ui.showNotification('Venta restaurada');
    }
    ...
    executeClearCart() {
        ...
        this.checkHeldSale(); // Update button state if needed
    ...
    showPaymentModal() {
        ...
        this.dom.paymentTotalUsd.textContent = formatCurrency(total);
        ...
        this.dom.paymentMethodSelect.value = '';

        // Trigger change to render initial fields
    ...
    renderPaymentMethods() {
        ...
        if (methods.length === 0) {
            this.dom.paymentMethodSelect.innerHTML = '<option value="">No hay métodos de pago</option>';
            return;
        }

        this.dom.paymentMethodSelect.innerHTML = methods.map(method =>
    ...
    handlePaymentMethodChange() {
        ...
        const method = this.paymentMethods.find(m => String(m.id) === String(methodId));
        ...
        let fieldsHtml = `
            <div class="mb-3">
                ...
                <input type="number" id="payment-amount" step="0.01" value="${this.cart.reduce((sum, item) => sum + (item.price * item.quantity), 0).toFixed(2)}"
                    class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-slate-500">
            </div>
        `;

        if (method.requireReference) {
            fieldsHtml += `
                <div class="mb-3">
                    <label class="block text-sm font-medium text-slate-700 mb-1">Referencia / Nro. Transacción</label>
    ...
    calculateChange() {
        ...
        if (this.dom.confirmPaymentBtn) {
            if (method === 'cash') {
                // For cash, require sufficient payment (allow small margin of error)
                this.dom.confirmPaymentBtn.disabled = change < -0.01;
            } else {
                // For non-cash payments, just require that an amount was entered
    ...
    setButtonIcon(button, dPath) {
        ...
        // Create SVG element using DOM API to avoid parsing issues
